...
class GateBlock(nn.Module):
    ...
    def forward(self, x):
        y = self.P1(x)
        y_norm = self.layernorm1(y.permute(0, 2, 3, 1)).permute(0, 3, 1, 2)
        y_3 = self.P2(y_norm)
        a, b, c = torch.chunk(y_3, 3, dim=1)
        h1 = self.H(a)
        h1 = self.layer_norm_fixed(h1.permute(0, 2, 3, 1)).permute(0, 3, 1, 2)
        h2 = self.H(b)
        h2 = self.layer_norm_fixed(h2.permute(0, 2, 3, 1)).permute(0, 3, 1, 2)
        t  = self.T(c)
        t = self.layer_norm_fixed(t.permute(0, 2, 3, 1)).permute(0, 3, 1, 2)
        out = (h1 - h2) * t
        out = self.layer_norm_fixed(out.permute(0, 2, 3, 1)).permute(0, 3, 1, 2)
        out = self.P3(out)
...
class GateCNN(nn.Module):
    def __init__(self, num_classes, num_gateblocks=8, input_channels=3, mid1_channels=32, mid2_channels=128):
        ...
        self.preprocess_depthwise = nn.Sequential(
            nn.Conv2d(16, 16, kernel_size=13, padding=6, groups=16, bias=False),
            nn.BatchNorm2d(16)
        )
        self.dropout75 = nn.Dropout2d(0.25)
        ...
        self.dropout50 = nn.Dropout2d(0.5)
        self.classifier = nn.Sequential(
            nn.Conv2d(mid2_channels, num_classes, kernel_size=1, bias=False),
    ...
    def forward(self, x):
        ...
        x_ln = self.input_layernorm(x.permute(0, 2, 3, 1)).permute(0, 3, 1, 2)
        x_proj = self.initial_conv(x_ln)  # (B, 16, H, W)
        x_soft = F.softmax(x_proj, dim=1)  # (B, 16, H, W)
        
        # 13x13 depthwise conv + BN.
        x_pre = self.preprocess_depthwise(x_soft)  # (B, 16, H, W)
        x_drop = self.dropout75(x_pre)  # (B, 16, H, W)
        x_drop_ln = self.mid_layernorm(x_drop.permute(0, 2, 3, 1)).permute(0, 3, 1, 2)
        x_cat = torch.cat([x_drop_ln, x_ln], dim=1)  # (B, 16+input_channels, H, W)
        
        x_gate = self.gateblocks(x_cat)
        
        x_final = self.final_layernorm(x_gate.permute(0, 2, 3, 1)).permute(0, 3, 1, 2)
        x_final = self.dropout50(x_final)
        ...
        logits = self.post_pool_layernorm(logits.permute(0, 2, 1)).permute(0, 2, 1)  # (B, num_classes, W)
